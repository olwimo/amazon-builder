...
GIT_VERSION="os-provided" # 'system' checks the base image first, else installs 'latest'
USE_PPA_IF_AVAILABLE="true"
...
if [ ${GIT_VERSION} = "os-provided" ] || [ ${GIT_VERSION} = "system" ]; then
    ...
    echo "Installing git from OS yum repository"
    # check_packages git
...
if ([ "${GIT_VERSION}" = "latest" ] || [ "${GIT_VERSION}" = "lts" ] || [ "${GIT_VERSION}" = "current" ]) && [ "${ID}" = "ubuntu" ] && [ "${USE_PPA_IF_AVAILABLE}" = "true" ]; then
    ...
    dnf install --allowerasing -y curl ca-certificates gnupg2 dirmngr
...
dnf install --allowerasing -y build-essential curl ca-certificates tar gettext libssl-dev zlib1g-dev libcurl?-openssl-dev libexpat1-dev
