...
updaterc() {
    if [ "${UPDATE_RC}" = "true" ]; then
        echo "Updating /etc/bashrc and /etc/zsh/zshrc..."
        if [ ! -f /etc/bashrc ]; then touch /etc/bashrc ; fi
        if [[ "$(cat /etc/bashrc)" != *"$1"* ]]; then
            echo -e "$1" >> /etc/bashrc
        fi
...
dnf install --allowerasing -y curl ca-certificates zip unzip sed
...
if [ ! -z "${ADDITIONAL_VERSIONS}" ]; then
    ...
    IFS=","
        ...
        for version in "${additional_versions[@]}"; do
            ...
            yes n | sdk_install java ${version} "\\s*" "(\\.[a-z0-9]+)*-${JDK_DISTRO}\\s*" ".*-[a-z]+$" "false"
        done
    IFS=$OLDIFS
