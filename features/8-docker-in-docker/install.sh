...
dnf install --allowerasing -y curl ca-certificates pigz iptables gnupg2 dirmngr wget
if ! type git > /dev/null 2>&1; then
    dnf install --allowerasing -y git
fi
...
if [ "${USE_MOBY}" = "true" ]; then
    ...
    curl https://download.docker.com/linux/fedora/docker-ce.repo | sed 's/$releasever/38/g' | sed 's/$basearch/x86_64/g' > /etc/yum.repos.d/docker-ce.repo
fi
...
if type docker > /dev/null 2>&1 && type dockerd > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        set +e # Handle error gracefully
            # dnf install --allowerasing -y moby-cli${cli_version_suffix} moby-buildx moby-engine${engine_version_suffix}
            dnf install --allowerasing -y docker
            if [ $? -ne 0 ]; then
        ...
        # Install compose
        # dnf install --allowerasing -y moby-compose || err "Package moby-compose (Docker Compose v2) not available for OS ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
    else
        dnf install --allowerasing -y docker-ce-cli${cli_version_suffix} docker-ce${engine_version_suffix}
        # Install compose
        dnf install --allowerasing -y docker-compose-plugin || echo "(*) Package docker-compose-plugin (Docker Compose v2) not available for OS ${ID} ${VERSION_CODENAME} (${architecture}). Skipping."
    fi
fi
